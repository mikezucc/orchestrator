...
import { ChevronLeft, Settings, Users, Key, Mail, UserPlus, Shield, Trash2, RefreshCw, ExternalLink, Cloud, FolderOpen } from 'lucide-react';
import { useNavigate, useSearchParams } from 'react-router-dom';
import ManageGCPProjectsModal from '../components/ManageGCPProjectsModal';
...
export default function OrganizationSettings() {
  ...
  const [showGCPProjectsModal, setShowGCPProjectsModal] = useState(false);
  ...
  return (
    <div className="min-h-screen bg-te-bg-light dark:bg-te-bg-dark">
            ...
            <div>
              ...
              {organization.gcpRefreshToken ? (
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2">Google Cloud Projects</label>
                    {organization.gcpProjectIds && organization.gcpProjectIds.length > 0 ? (
                      <div className="space-y-3">
                        <div className="bg-te-gray-50 dark:bg-te-gray-900/50 rounded-lg p-4">
                          <div className="space-y-2">
                            {organization.gcpProjectIds.map((projectId) => (
                              <div key={projectId} className="flex items-center space-x-2 text-sm">
                                ...
                                <span className="font-mono text-te-gray-700 dark:text-te-gray-300">{projectId}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                        {isOwnerOrAdmin && (
                          <button
                            onClick={() => setShowGCPProjectsModal(true)}
                      ...
                      <div className="space-y-3">
                        <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
                          <p className="text-sm text-yellow-800 dark:text-yellow-200">
                            No projects selected. Add projects to enable GCP resource management.
                          </p>
                        </div>
                        {isOwnerOrAdmin && (
                          <button
      ...
      {/* GCP Projects Modal */}
      <ManageGCPProjectsModal
        isOpen={showGCPProjectsModal}
        onClose={() => setShowGCPProjectsModal(false)}
        organizationId={organization?.id || ''}
        currentProjectIds={organization?.gcpProjectIds || []}
